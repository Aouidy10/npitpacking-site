    ...
    <header className="sticky top-0 z-40 bg-white shadow-sm">
      ...
      <div className="hidden md:flex bg-nauma-600 text-white/80 text-[11px]">
        <div className="max-w-6xl mx-auto px-4 w-full flex items-center justify-between h-8">
          ...
          <div className="flex items-center gap-5">
            <a href="tel:+212700700585" className="flex items-center gap-1 hover:text-white transition-colors">
            ...
            <button
              onClick={toggle}
              className="flex items-center gap-1 border border-white/30 rounded-full px-2.5 py-0.5 text-[11px] font-bold hover:bg-white/10 transition-colors"
                ...
                {CATEGORIES_CONFIG.map((cat) => (
                  <div key={cat.slug} className="flex-1 border-r border-gray-50 last:border-0">
                    ...
                    {cat.sousCats.length > 5 && (
                      ...
                      >
                        {t("nav.seeAll")} ({cat.sousCats.length})
                      </Link>
                    )}
                  </div>
                ))}
      ...
      <div className={clsx("md:hidden bg-white border-t border-gray-100 divide-y divide-gray-50", open ? "block" : "hidden")}>
        ...
        <div className="px-4 py-3 space-y-2">
          ...
          <button
            onClick={toggle}
            className="w-full flex items-center justify-center gap-2 border border-gray-200 rounded-lg py-2.5 text-sm font-bold text-gray-600 hover:bg-gray-50"
          >
            {lang === "fr" ? "🇲🇦 عربي — Basculer en arabe" : "🇫🇷 FR — Basculer en français"}
          </button>
        </div>
      </div>
